...
pub mod dma {
    ...
    pub fn slice_in_ram<T>(slice: &[T]) -> bool {
        let ptr = slice.as_ptr() as usize;
        ptr >= SRAM_LOWER && (ptr + slice.len() * core::mem::size_of::<T>()) < SRAM_UPPER
    }

    /// Return an error if slice is not in RAM.
    pub fn slice_in_ram_or<T, E>(slice: &[T], err: E) -> Result<(), E> {
...
pub mod io {
    ...
    pub trait ByteRead: Sync {
        ...
        /// Get an interrupt which will fire when a byte is ready to be read.
    ...
    pub trait ByteWrite: Sync {
        ...
        /// Get an interrupt which will fire when a byte has finished sending and the next is ready to be sent.
        fn irq(&mut self) -> &mut Self::Interrupt;
        ...
        /// Whether the event which triggers `irq` has fired.
        fn event_fired(&self) -> bool;

        /// Clear the event which triggers `irq`.
        fn clear_event(&self);
